...
const allowed = [
  ...
  { name: "Gemini 3.5", ok: true },
  { name: "Kimi K3", ok: false },
];
  ...
  return (
    <div className="rounded-2xl border border-border bg-white p-6 shadow-card">
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-full border border-green-300 bg-surface px-2.5 py-0.5 text-[11px] font-semibold text-green-700">
          Active
        </span>
      </div>
          ...
          {allowed.map((a) => (
            <div key={a.name} className="flex items-center justify-between">
              <span className="flex items-center gap-2 text-sm text-forest">
                <span className={a.ok ? "text-green-700" : "text-vermilion-700"}>
                  {a.ok ? <CheckCircle className="h-4 w-4" /> : <XCircle className="h-4 w-4" />}
                </span>
                {a.name}
              </span>
              <span className={`text-[11px] font-medium ${a.ok ? "text-muted" : "text-vermilion-700"}`}>
      ...
      <div className="mt-2 flex items-center justify-between rounded-xl bg-green-50 px-4 py-3">
        ...
        <span className="text-[11px] font-semibold text-green-700">Edit</span>
      </div>
    </div>
  );
}
    ...
    <section id="how-it-works" className="scroll-mt-16 border-t border-border bg-background">
      <div className="mx-auto max-w-6xl px-6 pb-16 pt-24">
        ...
        <h2 className="mx-auto mt-4 max-w-4xl text-center font-sans text-5xl font-semibold leading-[1.02] tracking-tight text-forest sm:text-6xl">
          One gateway.
          <br />
          Access for every <span className="text-vermilion-700">teammate.</span>
        </h2>
        {/* Steps (desktop row) */}
          ...
          {steps.map((s, i) => (
            <Reveal key={s.n} delay={i * 70} className="flex gap-4">
              ...
              <div>
                ...
                <p className="mt-1.5 text-sm leading-6 text-body">{s.desc}</p>
              </div>
            </Reveal>
          ))}
        </div>
        ...
        <div className="mt-10 hidden flex-col items-stretch gap-4 md:flex lg:flex-row lg:items-stretch">
          {/* Card 1 — providers */}
          <div className="flex-1 rounded-2xl border border-border bg-white p-6 shadow-card">
            <p className="text-sm font-bold text-forest">Your providers</p>
            <div className="mt-5 space-y-2.5">
              {providers.map((p) => (
                <div key={p.name} className="flex items-center justify-between rounded-xl border border-border px-3.5 py-2.5">
